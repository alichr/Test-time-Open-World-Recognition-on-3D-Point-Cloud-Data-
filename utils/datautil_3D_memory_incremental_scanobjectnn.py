...
import pandas as pd
from configs.scanobject_info import task_ids_total as tid, len_cls
    ...
    def __init__(self, root, task_num, classes, memory_classes, memory, folder, fewshot=0, sem_file=None, transform=None, phase=None):
      ...
      if self.folder=='train':
        ...
        if self.task_num==0:
          ...
          self.class_mapping={c:i+26 for i,c in enumerate(classes)}
          ...
          self.class_mapping={c:i+34 for i,c in enumerate(classes)} 
        ...
        if self.classes[self.files[i]['category']] in classes:
          ...
          if self.folder=='test':         
            if 0<=(self.class_mapping[self.classes[self.files[i]['category']]])<=25:
              ...
              task_label.append(2)
            elif 34<=(self.class_mapping[self.classes[self.files[i]['category']]])<=36:
              task_label.append(3)
