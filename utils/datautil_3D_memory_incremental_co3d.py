...
class PointCloudData(Dataset):
    def __init__(self, root_dir, folder="train"):
        ...
        # {'pcd_path': Path('E:/NSU/sfr1/.....'), 'category': '...', 'name': '...'}
    ...
    def __init__(self, root, task_num, classes, memory_classes, memory, folder, fewshot=0, sem_file=None, transform=None, phase=None):
      ...
      if self.folder=='train':
        ...
        if self.task_num==0:
          ...
          self.class_mapping={c:i+40 for i,c in enumerate(classes)} 
        elif self.task_num==5:
          self.class_mapping={c:i+45 for i,c in enumerate(classes)}
      ...
      for i in range(len(self.files)):
        if self.classes[self.files[i]['category']] in classes:
          ...
          #######
          if self.folder=='test':         
            if 0<=(self.class_mapping[self.classes[self.files[i]['category']]])<=24:
              task_label.append(0)
              ...
              task_label.append(3)
            elif 40<=(self.class_mapping[self.classes[self.files[i]['category']]])<=44:
              task_label.append(4)
            elif 45<=(self.class_mapping[self.classes[self.files[i]['category']]])<=49:
              task_label.append(5)
...
###### add sem
    ...
    def __getitem__(self, index):
        ...
        class_label = self.classes[str(names)]
...
class DatasetGen(object):
    ...
    def get(self, task_id, phase):
        ...
        self.train_set[task_id] = iPointCloudData(root=self.root, classes=task_ids[task_id], memory_classes=None, 
                                                sem_file=self.sem_file, memory=None, task_num=task_id, folder="train", 
        ...
        train_loader = torch.utils.data.DataLoader(self.train_set[task_id], batch_size=self.batch_size, num_workers=self.num_workers,
                                                    pin_memory=self.pin_memory,shuffle=True, drop_last=True)#, drop_last=True)
        test_loader = torch.utils.data.DataLoader(self.test_set[task_id], batch_size=self.batch_size, num_workers=self.num_workers,
                                                    pin_memory=self.pin_memory, shuffle=True, drop_last=True)
