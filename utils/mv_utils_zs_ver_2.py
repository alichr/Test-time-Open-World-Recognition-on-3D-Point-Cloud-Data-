...
class Grid2Image(nn.Module):
    ...
    def __init__(self):
        ...
        kn3d = get3DGaussianKernel(params['convxy'], params['convz'], sigma=params['convsigmaxy'],
                                   zsigma=params['convsigmaz']).to(device)
...
class Realistic_Projection_Learnable:
    ...
    def get_img(self, points, projection_angle):
        b, _, _ = points.shape
        _points = []
        for kk in range(b):
            angle = projection_angle[kk, 0:3].unsqueeze(0)
            angle2 = projection_angle[kk, 6:9].unsqueeze(0)
            translation = projection_angle[kk, 3:6].unsqueeze(0)
            rot_mat = euler2mat(angle).transpose(1, 2)
            rot_mat2 = euler2mat(angle2).transpose(1, 2)
            ...
            _points.append(transformed_points)
    ...
    def point_transform(points, rot_mat, rot_mat2, translation):
        ...
        # Create new tensors instead of modifying 'points' in-place
        transformed_points = torch.matmul(points, rot_mat)
        transformed_points = torch.matmul(transformed_points, rot_mat2)
        transformed_points = transformed_points - translation

        return transformed_points


class Realistic_Projection_Learnable_new(nn.Module):
    """For creating images from PC based on the view information.
    """

    def __init__(self):
        super(Realistic_Projection_Learnable_new, self).__init__()
        self.num_views = 1
        self.grid2image = Grid2Image()
