    ...
    def __init__(self, root, task_num, classes, memory_classes, memory, folder, fewshot=0, sem_file=None, transform=None, phase=None):
      ...
      if self.folder=='train':
        ...
        if self.task_num==0:
          ...
          self.class_mapping={c:i+50 for i,c in enumerate(classes)}  
        ...
        if self.classes[self.files[i]['category']] in classes:
          ...
          if self.folder=='test':         
            if 0<=(self.class_mapping[self.classes[self.files[i]['category']]])<=24:
              ...
              task_label.append(5)
            elif 50<=(self.class_mapping[self.classes[self.files[i]['category']]])<=54:
              task_label.append(6)
...
class DatasetGen(object):
    ...
    def get(self, task_id, phase):
        ...
        if task_id == 0:
            ...
            memory_classes=tid #[[i for i in range(0,26)],[i for i in range(26,30)],[i for i in range(30,34)],[i for i in range(34,37)]]
