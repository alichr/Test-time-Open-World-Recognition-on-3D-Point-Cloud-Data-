...
def main(opt):
    ...
    # load the data
    prototype = np.zeros((40, 512))
    sample_num = np.zeros((40))
    for t in range(0,5):
    ...
    for t in range(0,5):
        ...
        for epoch in range(1):
            ...
            for i, data in tqdm(enumerate(trainDataLoader, 0)):
                ...
                # select five samples per class
                num_samples_per_class = 5
                for jj in range(points.shape[0]):
                    dis = torch.cosine_similarity(fea_embedding[jj, :].unsqueeze(0), torch.from_numpy(prototype[target[jj], :]).to(device).unsqueeze(0))
                    if dis > Distance[target[jj]]:
                        class_samples = memory_bank_label == target[jj]
                        num_class_samples = torch.sum(class_samples)
                        if num_class_samples < num_samples_per_class:
                            memory_bank[target[jj], num_class_samples, :] = points[jj, :, :]
                            memory_bank_label[target[jj], num_class_samples] = target[jj]
                            Distance[target[jj]] = dis
                        else:
                            similarities = torch.cosine_similarity(fea_embedding[jj, :].unsqueeze(0), memory_bank[target[jj], :num_class_samples, :])
                            min_sim, min_idx = torch.min(similarities, dim=0)
                            if dis > min_sim:
                                memory_bank[target[jj], min_idx, :] = points[jj, :, :]
                                Distance[target[jj]] = dis
        # save the memory bank as numpy array
        np.save('memory_bank.npy', memory_bank.cpu().numpy())
        np.save('memory_bank_label.npy', memory_bank_label.cpu().numpy())
