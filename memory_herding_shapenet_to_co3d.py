...
def main(opt):
    ...
    memory_bank = torch.zeros((89, 1024,3)).to(device)
    memory_bank_label = torch.zeros((89, 1)).to(device)
   
    # load the data
    prototype = np.zeros((89, 512))
    sample_num = np.zeros((89))
    for t in range(0,11):
    ...
    for t in range(0,4):
        ...
        np.save('memory/memory_bank_shapenet_od3d.npy', memory_bank.cpu().numpy())
        np.save('memory/memory_bank_label_shapenet_od3d.npy', memory_bank_label.cpu().numpy())
