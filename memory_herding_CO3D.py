...
def main(opt):
    ...
    pointnet.load_state_dict(torch.load('cls/modelnet_scanobjectnn/pointnet_159.pth', map_location=device))
    ...
    for i in range(num_rotations):
        ...
        transform[str(i)].load_state_dict(torch.load('cls/modelnet_scanobjectnn/transform_159_%d.pth' % i, map_location=device))

    # load the Unet model
    unet = UNetPlusPlus().to(device)
    unet.load_state_dict(torch.load('cls/modelnet_scanobjectnn/unet_159.pth', map_location=device))
   
    # Step 4: Load the Relation Network
    relation = RelationNetwork(1536, 2048, 1024)
    relation = relation.to(device)
    relation.load_state_dict(torch.load('cls/modelnet_scanobjectnn/relation_159.pth', map_location=device))
    ...
    memory_bank = torch.zeros((50, 1024,3)).to(device)
    memory_bank_label = torch.zeros((50, 1)).to(device)
   
    # load the data
    prototype = np.zeros((50, 512))
    sample_num = np.zeros((50))
    for t in range(0,6):
    ...
    for t in range(0,4):
        ...
        np.save('memory/memory_bank_co3d.npy', memory_bank.cpu().numpy())
        np.save('memory/memory_bank_label_co3d.npy', memory_bank_label.cpu().numpy())
