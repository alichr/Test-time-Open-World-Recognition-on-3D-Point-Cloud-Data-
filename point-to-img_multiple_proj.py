...
class Transformation(torch.nn.Module):
    def __init__(self, num_rotations=10):
...
class CombinedConstraintLoss(nn.Module):
    def __init__(self, ortho_weight=1.0, scaling_weight=1.0, det_weight=1.0, num_rotations=10):
...
def main(opt):
    ...
    for epoch in range(opt.nepoch):
        ...
        for i, data in enumerate(trainloader):
            ...
            # extract depth map
            points = points.transpose(2, 1)
            depth_map = torch.zeros((points.shape[0] * 10, 3, 224, 224)).to(device)
            for m in range(10):
                print(transformation[:,m].shape)    
                depth_map[(m)*points.shape[0]:(m+1)*points.shape[0]] = proj.get_img(points, transformation[:,m])
            stop
