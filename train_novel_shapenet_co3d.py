...
def main(opt):
    ...
    curvenet.load_state_dict(torch.load('cls/shapenet_co3d/curvenet_40.pth', map_location=device))
    ...
    for i in range(num_rotations):
        ...
        transform[str(i)].load_state_dict(torch.load('cls/shapenet_co3d/transform_40_%d.pth' % i, map_location=device))

    # load the Unet model
    unet = UNetPlusPlus().to(device)
    unet.load_state_dict(torch.load('cls/shapenet_co3d/unet_40.pth', map_location=device))
   
    # Step 4: Load the Relation Network
    relation = RelationNetwork(1536, 2048, 1024)
    relation = relation.to(device)
    relation.load_state_dict(torch.load('cls/shapenet_co3d/relation_40.pth', map_location=device))

    
    #load the text features
    class_name = read_txt_file_class_name("class_name_shapenet_co3d.txt")
    prompts_shapenet = read_json_file("shapenet.json")
    prompts_co3d = read_json_file("CO3D.json")
    ...
    memory_bank = np.load('memory/memory_bank_shapenet_co3d.npy')
    memory_bank_label = np.load('memory/memory_bank_label_shapenet_co3d.npy')
    ...
    for t in range(0,11):
        ...
        for epoch in range(nepoch):
            ...
            for i, data in tqdm(enumerate(trainDataLoader, 0)):
                ...
                for j in range(num_category):
                    ...
                    tmp_1 = tmp_1.split(' ')
                    if j < 44:
                        tmp_2 = prompts_shapenet[tmp_1[1]]
                    else:
                        tmp_2 = prompts_co3d[tmp_1[1]]              
        ...
        task1, task2, task3, task4, task5, task6, task7, task8, task9, task10, task11, task1_total, task2_total, task3_total, task4_total, task5_total, task6_total, task7_total, task8_total, task9_total, task10_total, task11_total = [0] * 22
        tid_all = []
        for h in range(t+1):                
            tid_all += tid[h]
        ...
        for j, data in tqdm(enumerate(testDataLoader, 0)):
            ...
            if prediction == target.cpu().detach().numpy() and target.cpu().detach().numpy() < 39:
                task1 += 1
            if prediction == target.cpu().detach().numpy() and target.cpu().detach().numpy() >= 39 and target.cpu().detach().numpy() < 44:
                task2 += 1
            if prediction == target.cpu().detach().numpy() and target.cpu().detach().numpy() >= 44 and target.cpu().detach().numpy() < 49:
                task3 += 1
            if prediction == target.cpu().detach().numpy() and target.cpu().detach().numpy() >= 49 and target.cpu().detach().numpy() < 54:
                task4 += 1
            if prediction == target.cpu().detach().numpy() and target.cpu().detach().numpy() >= 54 and target.cpu().detach().numpy() < 59:
                task5 += 1
            if prediction == target.cpu().detach().numpy() and target.cpu().detach().numpy() >= 59 and target.cpu().detach().numpy() < 64:
                task6 += 1
            if prediction == target.cpu().detach().numpy() and target.cpu().detach().numpy() >= 64 and target.cpu().detach().numpy() < 69:
                task7 += 1
            if prediction == target.cpu().detach().numpy() and target.cpu().detach().numpy() >= 69 and target.cpu().detach().numpy() < 74:
                task8 += 1
            if prediction == target.cpu().detach().numpy() and target.cpu().detach().numpy() >= 74 and target.cpu().detach().numpy() < 79:
                task9 += 1
            if prediction == target.cpu().detach().numpy() and target.cpu().detach().numpy() >= 79 and target.cpu().detach().numpy() < 84:
                task10 += 1
            if prediction == target.cpu().detach().numpy() and target.cpu().detach().numpy() >= 84 and target.cpu().detach().numpy() < 89:
                task11 += 1
            if target.cpu().detach().numpy() < 39:
                task1_total += 1
            if target.cpu().detach().numpy() >= 39 and target.cpu().detach().numpy() < 44:
                task2_total += 1
            if target.cpu().detach().numpy() >= 44 and target.cpu().detach().numpy() < 49:
                task3_total += 1
            if target.cpu().detach().numpy() >= 49 and target.cpu().detach().numpy() < 54:
                task4_total += 1
            if target.cpu().detach().numpy() >= 54 and target.cpu().detach().numpy() < 59:
                task5_total += 1
            if target.cpu().detach().numpy() >= 59 and target.cpu().detach().numpy() < 64:
                task6_total += 1
            if target.cpu().detach().numpy() >= 64 and target.cpu().detach().numpy() < 69:
                task7_total += 1
            if target.cpu().detach().numpy() >= 69 and target.cpu().detach().numpy() < 74:
                task8_total += 1
            if target.cpu().detach().numpy() >= 74 and target.cpu().detach().numpy() < 79:
                task9_total += 1
            if target.cpu().detach().numpy() >= 79 and target.cpu().detach().numpy() < 84:
                task10_total += 1
            if target.cpu().detach().numpy() >= 84 and target.cpu().detach().numpy() < 89:
                task11_total += 1
        ...
        if task8_total > 0:
           print('task8:', task8/task8_total)
        if task9_total > 0:
           print('task9:', task9/task9_total)
        if task10_total > 0:
           print('task10:', task10/task10_total)
        if task11_total > 0:
           print('task11:', task11/task11_total)
