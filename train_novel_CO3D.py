...
def main(opt):
    ...
    pointnet.load_state_dict(torch.load('cls/co3d/pointnet_242.pth', map_location=device))
    ...
    for i in range(num_rotations):
        ...
        transform[str(i)].load_state_dict(torch.load('cls/co3d/transform_242_%d.pth' % i, map_location=device))

    # load the Unet model
    unet = UNetPlusPlus().to(device)
    unet.load_state_dict(torch.load('cls/co3d/unet_242.pth', map_location=device))
   
    # Step 4: Load the Relation Network
    relation = RelationNetwork(1536, 2048, 1024)
    relation = relation.to(device)
    relation.load_state_dict(torch.load('cls/co3d/relation_242.pth', map_location=device))
    ...
    for t in range(0,6):
        ...
        task1, task2, task3, task4, task5, task6, task1_total, task2_total, task3_total, task4_total, task5_total, task6_total = [0] * 12
        tid_all = []
        for h in range(t+1):                
            tid_all += tid[h]
