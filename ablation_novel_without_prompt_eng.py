...
def main(opt):
    ...
    curvenet.load_state_dict(torch.load('cls/shapenet_scanobjectnn/withoutprompteng/curvenet_40.pth', map_location=device))
    ...
    for i in range(num_rotations):
        ...
        transform[str(i)].load_state_dict(torch.load('cls/shapenet_scanobjectnn/withoutprompteng/transform_40_%d.pth' % i, map_location=device))

    # load the Unet model
    unet = UNetPlusPlus().to(device)
    unet.load_state_dict(torch.load('cls/shapenet_scanobjectnn/withoutprompteng/unet_40.pth', map_location=device))
   
    # Step 4: Load the Relation Network
    relation = RelationNetwork(1536, 2048, 1024)
    relation = relation.to(device)
    relation.load_state_dict(torch.load('cls/shapenet_scanobjectnn/withoutprompteng/relation_40.pth', map_location=device))
    ...
    memory_bank = np.load('memory/memory_bank_shapenet_scanobjectnn.npy')
    memory_bank_label = np.load('memory/memory_bank_label_shapenet_scanobjectnn.npy')
    ...
    for t in range(0,4):
        ...
        for epoch in range(nepoch):
            ...
            for i, data in tqdm(enumerate(trainDataLoader, 0)):
                ...
                prompts_train = read_txt_file("class_name_shapenet_scanobjectnn.txt")
                prompts_batch = []
                for j in range(opt.num_category):
                    tmp_1 = (prompts_train[tid[t][j]])
                    prompts_batch.append(tmp_1)
        ...
        for j, data in tqdm(enumerate(testDataLoader, 0)):
            ...
            if prediction == target.cpu().detach().numpy() and target.cpu().detach().numpy() < 44:
                task1 += 1
