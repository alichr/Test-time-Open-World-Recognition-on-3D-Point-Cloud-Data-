...
def main(opt):
    ...
    transform = Transformation(num_rotations=num_rotations)
    transform.to(device)
    ...
    for epoch in range(opt.nepoch):
        ...
        for i, data in tqdm(enumerate(trainDataLoader, 0)):
          ...
          #  print('view1:', transform_matrix[0,0,:,:].view(-1, 9))
           # print('view2:', transform_matrix[0,1,:,:].view(-1, 9))
           # print('---------------------------------------')
            
            
            loss_orthogonal = constraint_loss(transform_matrix.to(device))
           ...
           # text_embedding = F.normalize(text_embedding)
            ...
            loss = 0
            img_embedding_avg = (img_embedding[:48,:] + img_embedding[48:,:])/2
           ...
           # for kk in range(16):
            ...
            del points, target, points_fea, transform_matrix, depth_map, img_embedding, text_embedding, loss, logits, predicted
            torch.cuda.empty_cache()
        print('loss_orthogonal_weight:',loss_orthogonal_weight )    
